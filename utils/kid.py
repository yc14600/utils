...
def polynomial_mmd_averages(codes_g, codes_r, n_subsets=10, subset_size=1000,
    ...
    #subset_size = min(int(m/n_subsets),subset_size)
...
class KID_Evaluator(FID_Evaluator):

    def score(self,x_a,x_b,extractf=True,degree=3):
        if extractf:
            x_a = self.get_activations(x_a)
            x_b = self.get_activations(x_b)
        print('degree',degree)
        
        kid = polynomial_mmd_averages(x_a, x_b, output=sys.stdout, 
                                        n_subsets=20, subset_size=2000, 
                                        ret_var=False,degree=degree)
